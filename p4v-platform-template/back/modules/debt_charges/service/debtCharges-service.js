...
class DebtChargesService {
    ...
    async generateTaxNotificationById(request, reply) {
        try {
            ...
            console.log('📋 Main charge found:', mainCharge.id);
            
            // 2. ВИЗНАЧЕННЯ ТИПУ ОСНОВНОГО ПОДАТКУ
            const { taxType: primaryTaxType } = this.determineTaxTypeFromCharge(mainCharge);
            console.log('🎯 Primary tax type:', primaryTaxType);
            
            // 3. ПОШУК ВСІХ НАРАХУВАНЬ ПЛАТНИКА ПО ПІБ
            ...
            // 4. ⭐ НОВА ЛОГІКА: ГРУПУВАННЯ ПО ТИПАХ ПОДАТКІВ
            const chargesByTaxType = this.groupChargesByTaxType(allCharges);
            console.log('📊 Charges grouped by tax types:', 
                Object.keys(chargesByTaxType).map(type => 
                    `${type}: ${chargesByTaxType[type].length}`
                )
            );
            
            // 5. ⭐ СТВОРЕННЯ БЛОКІВ (PRIMARY ЗАВЖДИ ПЕРШИЙ)
            const taxBlocks = this.createTaxBlocks(chargesByTaxType, primaryTaxType);
            console.log(`📦 Created ${taxBlocks.length} tax blocks`);
            
            // 6. РОЗРАХУНОК ЗАГАЛЬНОЇ СУМИ
            const grandTotal = taxBlocks.reduce(
                (sum, block) => sum + block.totalAmount, 0
            );
            console.log(`💰 Grand total: ${grandTotal.toFixed(2)} грн`);
            
            // 7. ОТРИМАННЯ РЕКВІЗИТІВ
            ...
            // 8. ПОШУК ІНФОРМАЦІЇ ПРО БОРЖНИКА
            let debtorInfo = null;
            if (mainCharge.payer_name) {
                try {
            ...
            // 9. ⭐ СТВОРЕННЯ ЗБІРНОГО ОБ'ЄКТУ З БЛОКАМИ
            const combinedCharge = {
                ...mainCharge,
                taxBlocks: taxBlocks,        // Масив блоків податків
                grandTotal: grandTotal,      // Загальна сума всіх податків
                primaryTaxType: primaryTaxType,
                totalBlocksCount: taxBlocks.length
            };
            
            console.log('📄 Combined charge structure:', {
                payer: mainCharge.payer_name,
                primaryType: primaryTaxType,
                blocksCount: taxBlocks.length,
                grandTotal: grandTotal
            });
            
            // 10. ГЕНЕРАЦІЯ ДОКУМЕНТА
            ...
            await logRepository.createLog({
                ...
                action: 'GENERATE_DOC',
                client_addr: request?.ip,
                application_name: `Генерування податкового повідомлення для ${mainCharge.payer_name} (${taxBlocks.length} блоків податків)`,
                action_stamp_tx: new Date(),
                action_stamp_stm: new Date(),
                action_stamp_clk: new Date(),
                schema_name: 'ower',
            ...
            reply.header('Content-Disposition', 
                `attachment; filename=tax-notification-${safeFileName}-all-taxes.docx`
            );
    ...
    // ⭐ НОВИЙ МЕТОД: Групування нарахувань по типах податків
    groupChargesByTaxType(charges) {
        ...
        // Сортуємо кожну групу по даті (desc)
    ...
    // ⭐ НОВИЙ МЕТОД: Створення блоків податків
    createTaxBlocks(chargesByTaxType, primaryTaxType) {
        const blocks = [];
        
        // Маппінг типів на назви
        const taxNames = {
            land: 'земельного податку з фізичних осіб',
            non_residential: 'податку на нерухомість (не житлова) з фізичних осіб',
            residential: 'податку на нерухомість (житлова) з фізичних осіб',
            rent: 'оренди землі з фізичних осіб',
            mpz: 'мінімального податкового зобов\'язання (МПЗ)'
        };
        
        // Порядок блоків (після primary)
        const taxTypesOrder = ['land', 'non_residential', 'residential', 'rent', 'mpz'];
        
        // 1. ПЕРШИМ ЗАВЖДИ ЙДЕ PRIMARY БЛОК
        if (chargesByTaxType[primaryTaxType] && chargesByTaxType[primaryTaxType].length > 0) {
            const charges = chargesByTaxType[primaryTaxType];
            const totalAmount = charges.reduce((sum, c) => sum + (Number(c.amount) || 0), 0);
            
            blocks.push({
                taxType: primaryTaxType,
                taxName: taxNames[primaryTaxType],
                charges: charges,
            ...
            console.log(`✅ Primary block created: ${primaryTaxType} (${charges.length} charges, ${totalAmount.toFixed(2)} грн)`);
        }
        
        // 2. ДОДАЄМО ІНШІ БЛОКИ (ЯКЩО Є НАРАХУВАННЯ)
        let blockNumber = 2;
        taxTypesOrder.forEach(taxType => {
            // Пропускаємо primary (він вже доданий)
            if (taxType === primaryTaxType) return;
            
            const charges = chargesByTaxType[taxType];
            if (charges && charges.length > 0) {
                ...
                console.log(`➕ Additional block created: ${taxType} (${charges.length} charges, ${totalAmount.toFixed(2)} грн)`);
            }
        });
